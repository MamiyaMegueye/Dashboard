    ...
    while True:
        try:
            ...
            if df.empty:
                ...
                try:
                    # Upsert raw_cloture_secteur
                    con.execute("DELETE FROM raw_cloture_secteur")
                    con.execute(
                        "INSERT INTO raw_cloture_secteur SELECT * FROM df"
                    )

                    # 3. Reconstruire les marts cloture
                    await asyncio.to_thread(build_marts_cloture, con)
                    ...
                    if nb_new > 0:
                        ...
                        new_alerts = con.execute("""
                            SELECT
                                ...
                                a.pct_apres,
                                a.detected_at,
                                s.SECT_LIBLT  AS SECT_LIB,
                                st.STR_LIB_LT AS CENTRE_LIB
                            FROM alerts_log a
                            LEFT JOIN v_secteur_cycle s
                                ON s.SECT_ID = a.SECT_ID
                               AND s.STR_ID  = a.STR_ID
                            LEFT JOIN v_structure st
                                ON st.STR_ID = a.STR_ID
